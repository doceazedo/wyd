...
const createSession = (cookies: Cookies = {}) =>
	new Session({ baseUrl: BASE_URL, impersonate: 'chrome', cookies, timeout: 30 });
...
const signIn = async (): Promise<Cookies> => {
	...
	try {
		...
		const token = parse(form.text)
			...
			?.getAttribute('value');
		if (!token) throw new StorygraphError(`could not read the sign in form (HTTP ${form.status})`);
...
const requestProfile = async (path: string, refresh: boolean): Promise<string | null> => {
	const session = createSession(await getCookies(refresh));
	try {
		const resp = await session.get(path);
		if (resp.status !== 200) {
			throw new StorygraphError(`The StoryGraph responded with HTTP ${resp.status} for ${path}`);
		}
...
const fetchProfile = async (user: string): Promise<string> => {
	...
	const html = (await requestProfile(path, false)) || (await requestProfile(path, true));
	if (!html) throw new StorygraphError('could not stay signed in to The StoryGraph');
...
const clean = (value: string | undefined | null) => {
	const WHITESPACE = ['\n', '\r', '\t'];
	const flattened = WHITESPACE.reduce((text, char) => text.split(char).join(' '), value || '');
	return flattened.split(' ').filter(Boolean).join(' ');
};

const isDigits = (text: string) =>
	text.length > 0 && [...text].every((char) => (char >= '0' && char <= '9') || char === ',');

const toNumber = (value: string | undefined | null) => {
	const digits = [...(value || '')].filter((char) => char >= '0' && char <= '9').join('');
...
const toCount = (value: string) => {
	...
	if (open !== -1 && text.endsWith(')')) return toNumber(text.slice(open + 1, -1));
...
const sectionSlug = (href: string | undefined | null, username: string): SectionSlug | null => {
	const [slug, user, ...rest] = pathSegments(href);
	if (!slug || !user || rest.length) return null;
	if (!(slug in SECTIONS)) return null;
	if (decodeURIComponent(user).toLowerCase() !== username.toLowerCase()) return null;
...
const parseTaste = (spans: HTMLElement[]) => {
	const values = (prefix: string) => {
		const span = spans.find((candidate) => clean(candidate.text).startsWith(prefix));
		return span ? span.querySelectorAll('span').map((value) => clean(value.text)) : [];
	};
	...
	return { genre: genre || null, moods, pace: pace || null, length: length || null };
};
...
const parseProfile = (root: HTMLElement, requested: string) => {
	...
	const handle = pane
		...
		.find((text) => text.length > 1 && text.startsWith('@') && !text.includes(' '));
	const username = handle ? handle.slice(1) : requested;

	const books = Object.fromEntries(
		Object.values(SECTIONS).map((key) => [key, [] as Book[]])
	) as Record<SectionKey, Book[]>;
	const counts = Object.fromEntries(Object.values(SECTIONS).map((key) => [key, null])) as Record<
		SectionKey,
		number | null
	>;
	...
	root.querySelectorAll('a[href]').forEach((anchor) => {
		...
		sectionBooks(anchor, username).forEach((link) => {
			...
			if (book && !books[key].some((existing) => existing.id === book.id)) books[key].push(book);
		});
	});

	const readAllTime = toNumber(root.querySelector('[aria-label*="books read all time"]')?.text);
	...
	return {
		user: {
			...
			username,
			name:
				clean(pane?.querySelector('.profile-badges')?.parentNode?.querySelector('p')?.text) || null,
			pronouns: clean(pane?.querySelector('span[title="Pronouns"]')?.text) || null,
			avatar: pane?.querySelector('img[alt$="profile picture"]')?.getAttribute('src') || null,
		...
		counts: {
			...
			readThisYear: toNumber(root.querySelector('[aria-label*="books read this year"]')?.text)
		},
